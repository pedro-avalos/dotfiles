...
class MouseButton(StrEnum):
    """Represents the mouse button names that Qtile uses."""
...
main_widgets = [
    ...
    widget.TextBox(
        text="",
        mouse_callbacks={MouseButton.LEFT: lazy.spawn(LAUNCHER_CMD)},
        **{
            **widget_defaults,
    ...
    widget.Clock(
        format=" %Y/%m/%d (%a)",
        mouse_callbacks={MouseButton.LEFT: lazy.spawn(CAL_CMD)},
        **{**widget_defaults, "font": fonts["icons"]},
    ),
    widget.Clock(
        format=" %H:%M",
    ...
    widget.Volume(
        fmt="󱄠 {}",
        mouse_callbacks={MouseButton.RIGHT: lazy.spawn(VOL_CTRL_CMD)},
        **{**widget_defaults, "font": fonts["icons"]},
    ),
    (
        widget.Battery(
